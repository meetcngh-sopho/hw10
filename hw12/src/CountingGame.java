...
public class CountingGame
{
    ...
    public static void main(String args[])
    {
        ...
        players=new AList<Integer>(max);//New Alist for players
        for(int i=0;i<max;i++)
        {
        	players.add(i+1);
        }
   ...
   public static int doRhyme(ListInterface<Integer> players, ListInterface<String> rhyme, int startAt)
    {
	   int i=players.getLength(); //Length of player's list
	   int j=0;
	   for(int k=1;k<=rhyme.getLength();k++)
	   {
		   //Printing one word of the rhyme for each of the players repetedly until the rhyme finishes.
        	System.out.println("Player "+players.getEntry((j % i)+1)+": "+rhyme.getEntry(k));
        	j++;
        }
	   j--;
	   //Deleting the player who says it last from the player's list.
